...
Having defined the parameters, I^[I took a considerable amount of inspiration from this post.] initiate an empty list called `tweets_list` which is then appended in a for loop that breaks after `10000` iterations.
...
```{python write_csv}
#| eval: false
file = '../data/queries/2023-01/{keywords}_{start}_{end}.csv'.format(keywords=keywords, start=start, end=end)
...
```{python write_csv_2}
file = '../data/queries/2023-01/mvfw metaverse fashion week_2022-02-23_2022-04-27.csv'
